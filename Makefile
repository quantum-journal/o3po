...
test-%: $(shell find . -type f -name '*.php') setsttysizenonzero
	@if [ "$(shell phpunit --version | head -n 1 | sed -n 's/[^0-9]*\([0-9]*\)\..*/\1/p')" -ge "10" ]; then\
		$(PHPUNITCOMMAND) --coverage-clover=coverage.xml --display-deprecations --display-warnings --display-errors --display-phpunit-deprecations --coverage-filter o3po --coverage-html=coverage-html --bootstrap tests/resources/bootstrap.php --test-suffix 'est.php' tests/;\
	else\
