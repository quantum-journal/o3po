...
{       
    ...
    static public function parse_bbl( $bbl ) {
        ...
        preg_match('/% \$ biblatex bbl format version ([0-9.]*) \$/' , $bbl, $version);
        if( !empty($version[1]))
            $biblatex = $version[1];
        else
            $biblatex = false;
...
//      $entries = preg_split('#\\\\bibitem\s*(?:(?=\[((?:[^\[\]]++|\[(?1)\])*)\])\s*\[(?1)\]|)\s*(?=\{((?:[^{}]++|\{(?2)\})*)\})\s*{(?2)}#', $bbl);
            ...
            foreach($entries as $n => $entry) {	
                ...
                if(strpos($entry, 'begin{thebibliography}' ) || strpos($entry, 'begin{references}' ) || empty($entry) || ($n === 0 && !$contains_thebibliography_environment && !$contains_references_environment) ) continue;
