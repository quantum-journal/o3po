...
abstract class O3PO_PublicationType {
    ...
    protected function save_meta_data( $post_id ) {
		...
		if ($old_doi_suffix === $new_doi_suffix)
			...
			if ( !$this->doi_suffix_stil_free($new_doi_suffix) )
				$new_doi_suffix = '';
    ...
    protected function validate_and_process_data( $post_id ) {
        ...
        $highest_pages_info = static::get_post_type_highest_pages_info( $post_id, array($post_type) );
        ...
        $pages_still_free_info = static::pages_still_free_info( $post_id, $pages, array($post_type) );
...
//        $json_array["bibjson"]["journal"]["end_page"] = ?;
    ...
    protected function the_admin_panel_date_volume_pages( $post_id ) {
        ...
        if( empty( $pages ) ) {
			$highest_pages_info = static::get_post_type_highest_pages_info( $post_id, array($this->get_publication_type_name()) );
			$highest_pages = $highest_pages_info['pages'];
